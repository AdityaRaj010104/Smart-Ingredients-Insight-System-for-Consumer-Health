...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      const isDemoLogin = 
        (formData.identifier === DEMO_CREDENTIALS.email || formData.identifier === DEMO_CREDENTIALS.username) && 
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-gray-200">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold shadow-lg hover:shadow-xl hover:shadow-emerald-500/50 transition-all"
            >
              {isLoading ? (
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-2xl">
          <p className="text-sm text-blue-800 text-center mb-2 font-semibold flex items-center justify-center gap-2">
            <Sparkles className="w-4 h-4" />
            Try Demo Account
          </p>
          <div className="text-xs text-blue-700 text-center space-y-1">
            <p>Email: <span className="font-mono font-semibold">{DEMO_CREDENTIALS.email}</span></p>
            <p>Username: <span className="font-mono font-semibold">{DEMO_CREDENTIALS.username}</span></p>
            <p>Password: <span className="font-mono font-semibold">{DEMO_CREDENTIALS.password}</span></p>
          </div>
        </div>
